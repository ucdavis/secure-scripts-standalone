...
//	$recruit_upd = "";
	...
	if ($fp = fopen("/var/www/non_www/.my.cnf", "r"))
	{
		...
		if ($dbok = ($link = new mysqli($host_name, $user_name, $password, "cns")))
		{
			...
			{
					$q_insert = "UPDATE tea_recruit SET tea_subscribe = \"0\" WHERE email = \"$email\" ";
...
// if all of the required fields are entered clean the text fields and submit to the database
				...
				if ($valid_form == true)
				{
					...
					{
						if ($tea_subscribe == "2") $unsub_date = $sub_date; //if unsubscribing set the unsubscribe date
					...
					unsub_date = \"$unsub_date\", agency = \"$agency\", post_coalition = \"$post_coalition\"
					WHERE tea_id = \"$tea_id\" ";
					...
					if (mysqli_affected_rows($link) >= 1)
					{
						...
						print "<blockquote>";
						print "Your information has been updated.  Thank you for your interest in the Tobacco Evaluator Alliance (TEA).<p>
						<a href= \"http://tobaccoeval.ucdavis.edu/tea\">Go to the TEA web page</a>";
						print "</blockquote>";
					}
					else
...
//				print "<b><font size= \"2px\">To register your interest in the UCD MPH program, please complete and submit this form:</font></b>";
				print "<h2>Please complete and submit this form to subscribe to the Tobacco Evaluator Alliance (TEA) Contact List.  <br>TEA will use this information to contact you about upcoming meetings and events related to our Community of Practice for Evaluation.</h2>";
				...
				if ($recruit_upd)
				{
					...
					$q_alum = "SELECT * from tea_recruit WHERE tea_id = $recruit_upd";
				...
				print "<form name= \"info_req\" method= \"post\" action= \"tea_subscribe.php\">";
...
//						if ($minor_subscribe == 1) print "checked";
						print "checked";
						print ">&nbsp;&nbsp;<strong>I want to subscribe to the TEA Contact list.
						</strong></td>";
					print "</tr>";
					...
					if ($recruit_upd)
					{
						print "<td align= \"left\"><input type= \"submit\" name= \"sub_recruit\" value= \"Update Information\">";
...
// THIS IS THE SECTION TO UNSUBSCRIBE FROM THE LIST
				...
				print "<form name= \"unsubscribe\" method= \"post\" action= \"tea_subscribe.php\">";
//				print "<table border= \"0\" align= \"left\" cellspacing= \"2\" cellpadding= \"4\">";
