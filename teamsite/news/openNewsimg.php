...
//     $replaces = array(" ", "", "'", "", "", "<", ">");
	...
	if ($dbok = ($link = new mysqli($host_name, $user_name, $password, "dept"))) 
	{
		...
		$q_select = "SELECT newsimg_link, newsimg_name FROM news_features WHERE newsimg_hash LIKE '$getDoc' ";
		
		$r_select = mysqli_query($link, $q_select);
		$row_select = mysqli_fetch_array($r_select, MYSQLI_ASSOC);
		
		$fileName = $row_select['newsimg_name'];
		$fileLocation = $row_select['newsimg_link'];
