...
//	Construct page advance
		$display_limit = 4;
		
		$q_count = "SELECT count(news_id) FROM  news_features WHERE public_display = \"1\" ";
		$r_count = mysqli_query($link, $q_count);
		$row_count = mysqli_fetch_array($r_count, MYSQLI_NUM);
		$total_count = $row_count[0];
		
		if(isset($_GET["page"]))
		{
			$page = $_GET["page"] + 1;
			$offset = $display_limit * $page;
		}
		else
		{
			$page = 0;
		...
		$leftover_news = $total_count - ($page * $display_limit);
		
// end Construct page advance
		
//		$q_news = "SELECT * FROM news_features WHERE public_display = \"1\" ORDER BY top_rank DESC, rank LIMIT 10 ";
		$q_news = "SELECT * FROM news_features WHERE public_display = \"1\" ORDER BY top_rank DESC, rank LIMIT $offset, $display_limit ";
...
//				print "<tr><td colspan=\"2\"><hr></td></tr>";
			...
			// Next page links here
			
			if (($page > 0) && ($leftover_news > $display_limit))
			{
				$last = $page - 2;
				echo "<tr><td colspan=\"2\"><a href=\"fac_news.php?page=$last\">Previous</a> | ";
				echo "<a href=\"fac_news.php?page=$page\">Next</a></td></tr>";	
			}
			else if ($page == 0)
			{
				echo "<tr><td colspan=\"2\"><a href=\"fac_news.php?page=$page\">Next</a></td></tr>";
			}
			else if  (($page > 0) && ($leftover_news <= $display_limit))
			{
				$last = $page - 2;
				echo "<tr><td colspan=\"2\"><a href=\"fac_news.php?page=$last\">Previous</a></td></tr>";	
			}
