...
<!-- Start Main Content Column -->
	...
	<p><font color="#002666" style="font-size:13px; font-weight:bold">Please complete and submit this form to subscribe to the Public Health Minor Mailing List</font></p>
...
//	$recruit_upd = "";
	...
	if(isset($_POST['minor_id'])) $minor_id = $_POST['minor_id'];
	...
	if(isset($_POST['minor_subscribe'])) $minor_subscribe = $_POST['minor_subscribe'];
	...
	if ($fp = fopen("/var/www/non_www/.my.cnf", "r"))
	{
		...
		if ($dbok = ($link = new mysqli($host_name, $user_name, $password, "sph")))
		{
          ...
          $q_insert = "UPDATE minor_recruit SET minor_subscribe = \"0\", unsub_date = \"$unsub_date\" WHERE email = \"$email\" ";
					$r_insert = mysqli_query($link, $q_insert);

					if (mysqli_affected_rows($link) >= 1)
					{
						print "$email has been unsubscribed from the PHS Minor mailing list.";
					}
					else
					{
			...
			{
				print "The Public Health Minor Listserv information has not been submitted.<p>
				<a href= \"https://health.ucdavis.edu/phs/education/minor_mail.html\">Return to the Public Health Minor home page.</a>";
...
// if all of the required fields are entered clean the text fields and submit to the database
				...
				if ($valid_form == true)
				{
					...
					{
						if ($minor_subscribe <> "1") $minor_subscribe = "0";

					$q_insert = "INSERT INTO minor_recruit
					...
					minor_subscribe, sub_date)
					VALUES
					(\"$fname\", \"$lname\", \"$email\", \"$hear_university\", \"$hear_fb\", \"$hear_linkd\", \"$hear_twitter\", \"$hear_refer\", \"$hear_alumni\", \"$hear_website\",
					\"$hear_other\", \"$hear_other_spec\", \"$enroll_yr\", \"$enroll_qtr\", \"$minor_subscribe\", \"$sub_date\")";
					} // end if sub_recruit == submit information

					if ($sub_recruit == "Update Information")
					{
						if ($minor_subscribe == "2") $unsub_date = $sub_date; //if unsubscribing set the unsubscribe date

					$q_insert = "UPDATE minor_recruit SET
					...
					hear_other = \"$hear_other\", hear_other_spec = \"$hear_other_spec\", enroll_yr = \"$enroll_yr\", enroll_qtr = \"$enroll_qtr\", minor_subscribe = \"$minor_subscribe\",
					...
					if (mysqli_affected_rows($link) >= 1)
					{
						...
						print "<blockquote>";
						print "Your information has been updated.  Thank you for your interest in the UC Davis PHS Minor.<p>
						<a href= \"https://health.ucdavis.edu/phs/education/minor_mail.html\">Return to the Public Health Minor home page</a>";
						print "</blockquote>";
...
//						$cc_email = "PHSInstAffairs@phlistserv.ucdavis.edu";
						$subject = "Thanks for your interest in the Public Health Undergraduate Minor at UC Davis!";
...
//				print "<b><font size= \"2px\">To register your interest in the UCD MPH program, please complete and submit this form:</font></b>";
				...
				if ($recruit_upd)
				{
					...
					$q_alum = "SELECT * from minor_recruit WHERE minor_id = $recruit_upd";
					...
					$minor_subscribe = $row_alum['minor_subscribe'];
				} // end if recruit_upd


				print "<form name= \"info_req\" method= \"post\" action= \"minor_recruit.php\">";
				print "<table border= \"0\" align= \"left\" cellspacing= \"2\" cellpadding= \"4\">";
					...
					print "<tr>";
						...
						print ">&nbsp;&nbsp;Referred by a Student</td>";
...
//					print "</tr>";
					...
					print "<tr>";
						print "<td colspan= \"2\" class= \"recruit\"><input type= \"checkbox\" name= \"minor_subscribe\" value= \"1\" ";
//						if ($minor_subscribe == 1) print "checked";
						print "checked";
						print ">&nbsp;&nbsp;<strong>Do you want to subscribe to the Public Health Minor mailing list?
						</strong></td>";
					print "</tr>";
...
// THIS IS THE SECTION TO UNSUBSCRIBE FROM THE LIST
				...
				print "<form name= \"unsubscribe\" method= \"post\" action= \"minor_recruit.php\">";
				print "<table border= \"0\" align= \"left\" cellspacing= \"2\" cellpadding= \"4\">";
