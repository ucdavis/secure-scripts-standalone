...
//	include "/includes/common.lib.php";
	...
	if(isset($_POST['mr_id'])) $mr_id = $_POST['mr_id'];
	...
	if(isset($_POST['mph_subscribe'])) $mph_subscribe = $_POST['mph_subscribe'];
	...
	if ($fp = fopen("/var/www/non_www/.my.cnf", "r"))
	{
		...
		if ($dbok = ($link = new mysqli($host_name, $user_name, $password, "sph")))
		{
			...
			if ($sub_recruit == "Unsubscribe")
			{
					...
					$q_insert = "UPDATE mph_recruit SET mph_subscribe = \"0\", unsub_date = \"$unsub_date\" WHERE email = \"$email\" ";
					$r_insert = mysqli_query($link, $q_insert);

					if (mysqli_affected_rows($link) >= 1)
					{
						print "$email has been unsubscribed from the MPH Interest mailing list.";
					}
					else
					{
			...
			{
				print "No information has been sent to the MPH Program database.<p>
				<a href= \"https://health.ucdavis.edu/phs/education/mph/index.html\">Return to the MPH Program home page.</a>";
...
// if all of the required fields are entered clean the text fields and submit to the database
				...
				if ($valid_form == true)
				{
					...
					if ($mph_subscribe <> "1") $mph_subscribe = "0";


					if ($sub_recruit == "Submit Information")
					{
					$q_insert = "INSERT INTO mph_recruit
					...
					more_courses, more_other, more_other_spec, high_degree, sub_date, mph_subscribe)
					VALUES
					(\"$aoe_epm\", \"$aoe_nut\", \"$aoe_eoh\", \"$aoe_hzid\", \"$aoe_vph\", \"$aoe_gph\", \"$aoe_other\",
					...
					{
					$q_insert = "UPDATE mph_recruit SET
					...
					mph_subscribe = \"$mph_subscribe\" WHERE mr_id = \"$mr_id\" ";
					...
					if (mysqli_affected_rows($link) >= 1)
					{
						...
						print "<blockquote>";
						print "Your information has been submitted.  Thank you for your interest in the UC Davis MPH Program.<p>
						<a href= \"https://health.ucdavis.edu/phs/education/mph/index.html\">Return to the UC Davis MPH Program home page</a>";
						print "</blockquote>";
...
//				print "<b><font size= \"2px\">To register your interest in the UCD MPH program, please complete and submit this form:</font></b>";
				...
				if ($recruit_upd)
				{
					...
					$q_alum = "SELECT * from mph_recruit WHERE mr_id = $recruit_upd";
					...
					$mph_subscribe = $row_alum['mph_subscribe'];
				} // end if recruit_upd


				print "<form name= \"info_req\" method= \"post\" action= \"recruit.php\">";
				print "<table border= \"0\" align= \"left\" cellspacing= \"2\" cellpadding= \"4\">";
					...
					print "<tr>";
						...
						print ">&nbsp;&nbsp;Env & Occupational Health</td>";
...
//					print "</tr>";
					...
					print "<tr>";
						print "<td colspan= \"2\" class= \"recruit\"><b>5. Current Employment Status</b></td>";
					print "</tr>";
					...
					print "<tr>";
						print "<td colspan= \"2\" class= \"recruit\"><b>6. How did you learn about the UC Davis MPH Program?</b>&nbsp;(check all that apply)</td>";
					print "</tr>";
...
//					print "</tr>";
					...
					print "<tr>";
						print "<td colspan= \"2\" class= \"recruit\"><b>7. I am interested in receiving more information about:</b>&nbsp;(check all that apply)</td>";
					print "</tr>";
					...
					print "<tr>";
						print "<td colspan= \"2\" class= \"recruit\"><input type= \"checkbox\" name= \"mph_subscribe\" value= \"1\" ";
//						if ($mph_subscribe == 1) print "checked";
						print "checked";
						print ">&nbsp;&nbsp;<strong>8. Do you want to subscribe to the MPH Interest list?
						</strong></td>";
					print "</tr>";
				...
				// THIS IS THE SECTION TO UNSUBSCRIBE FROM THE LIST
								...
								print "<form name= \"unsubscribe\" method= \"post\" action= \"recruit.php\">";
								print "<table border= \"0\" align= \"left\" cellspacing= \"2\" cellpadding= \"4\">";
