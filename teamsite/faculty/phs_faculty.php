...
?>
  ...
  <?php
	...
	while ($facstaff < 3) {
		...
		if ($facstaff == 1) 
		{
			$sql = "select * from people where id like \"932\" and listing like \"%=1=%\" order by lname";	
		}

		elseif ($facstaff == 2) 
		{
			$sql = "select * from people where id not like \"932\" and staff_type not like \"acares\" and listing like \"%=1=%\" and emeritus not like \"1\" order by lname";
		}

		elseif ($facstaff == 3) 
		{
			$sql = "select * from people where id not like \"932\" and staff_type not like \"acares\" and listing like \"%=1=%\" and emeritus like \"1\" order by lname";
		}
	?>
  ...
  <?php
		...
		if ($result = mysqli_query($link, $sql)) {
			while ($line2 = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
				...
				if ((strlen($med_url) == 0) && (strlen($old_url) >= 1))
				{
					$med_url = $old_url;
					$bio_url = $old_url;
					$dont_append = 1;
...
<?php
				if ($emeritus_heading == "3")
				{
					print "<tr><td>&nbsp;</td></tr>";
					print "<tr><td colspan = \"2\"><span class=\"emeritus-head\">Emeritus Faculty</span></td></tr>";
					print "<tr><td>&nbsp;</td></tr>";
					$emeritus_heading++;
				} // end if
?>
